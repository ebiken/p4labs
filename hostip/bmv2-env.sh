...
create_network () {
    ...
    do
        HOST="host$num"
        VETH="veth$num"
        PORT="p$num" # another side of VETH pair
        echo "creating $HOST $VETH $PORT 192.168.0.$num/24 db8::$num/64"
        # Create network namespaces
        run ip netns add $HOST
        # Create veth and assign to host
        run ip link add $VETH type veth peer name $PORT
        ...
        run ip netns exec $HOST ip addr add 192.168.0.$num/24 dev $VETH
...
destroy_network () {
    ...
    do
        HOST="host$num"
        #veth will be removed when pair veth(p) is deleted
        #VETH="veth$num"
        PORT="p$num"
        run ip link del dev $PORT
...
Note:
    IPv4 address will be 192.168.0.<num>/24
